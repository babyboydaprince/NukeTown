...
# Read proxy list
def getProxys(file="modules/jackphone/proxy.json"):
...
class Service:
    ...
    def sendMessage(self, phone):
        ...
        # Create suffixes
        okay = f"{Fore.YELLOW}Service("
        "{getDomain(url)}) >> Message sent!{Fore.RESET}"
        error = f"{Fore.MAGENTA}Service("
        "{getDomain(url)}) >> Failed to sent message!{Fore.RESET}"
        ...
        elif dataType == payload['url']:
            try:
                request = request.prepare()
                r = session.send(request, timeout=self.timeout,
                                 proxies=self.proxy)
