    ...
    for _ in range(4):
        for index, sock in enumerate(sockets):
            try:
                ...
                print(
                    f"{Fore.RED}[-] {Fore.MAGENTA}Failed \
                        to send keep-alive headers{Fore.RESET}"
                )
                sockets.remove(sock)
            else:
                print(
                    f"{Fore.GREEN}[+] {Fore.YELLOW}Sending keep-alive \
                        headers to {'{}:{}'.format(*target)} from \
                            socket {index + 1}. {Fore.RESET}"
                )
